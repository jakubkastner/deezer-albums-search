...
namespace deezer
{
	public partial class Form1 : Form
	{
		...
		private void ZiskejAlba(string adresa, string interpret, bool smaz, bool vsechnyReleasy, bool featurings)
		{
			...
			if (chybaJson.JeChyba)
			{
				if (chybaJson.Kod == 4)
				{
					ZiskejAlba(adresa, interpret, smaz, vsechnyReleasy, featurings);
				}
				return;
			}
			...
			foreach (AlbumInformace nalezeneAlbum in seznamNalezenychAlb.data)
			{
				...
				if (vsechnyReleasy || typAlbumu == "album" || typAlbumu == "ep")
				{
					...
					var album = new Album(nalezeneAlbum.id);
					if (!featurings && !album.Interpreti.Contains(interpret, StringComparer.OrdinalIgnoreCase))
					{
						// interpret je pouze v songu na featuringu a vyhledávání featuringu je vypnuto
						continue;
					}
					nalezenaAlba.Add(album);
			...
			if (!String.IsNullOrEmpty(seznamNalezenychAlb.next))
			{
				...
				ZiskejAlba(seznamNalezenychAlb.next, interpret, false, vsechnyReleasy, featurings);
			}
		}
		...
		private void backgroundWorker1_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
		{
			...
			// získá ba umělce
			var adresa = "https://api.deezer.com/search/album?q=";
			if (!string.IsNullOrEmpty(umelec))
			{
				adresa += " artist:\"" + umelec + "\" ";
			}
			...
			ZiskejAlba(adresa, umelec, true, checkBox1.Checked, checkBox2.Checked);
		}
